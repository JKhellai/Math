...
函数最自然的玩法是**串起来用**：一个函数的输出，喂给下一个函数当输入。「先求平方、再加一」「先取等价类、再做别的」——数学里的复杂操作几乎都是简单操作的接力。这一节先把「接力」本身定成一个运算，摸清它的脾气；然后回应上一节末尾的悬念——**什么样的函数，箭头能走回去**。两件事到头来是一件：「走回去」必须用「接力」才说得清楚。
